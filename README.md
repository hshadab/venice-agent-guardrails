...
To upgrade that to "this specific response is provably from the attested enclave", pass `verifySignature: true` to `attestedChat`. Venice's TEE signs each completion with a secp256k1 key whose Ethereum address is the attestation's `signing_address`; the client fetches that signature (`GET /tee/signature?request_id=<completion id>`), recovers the signer from the EIP-191 signature, and rejects (fail-closed) unless it matches `signing_address`. The recovery is verified against Venice's published test vector in `test/signature.test.js`.

```js
const r = await venice.attestedChat({ messages, verifySignature: true });
// throws on mismatch; otherwise:
r.attestation.signatureVerified; // true
```

Still out of scope: full client-side Intel TDX quote verification against Intel collateral (parsing `mrtd`/`rtmr*`, checking the certificate chain). `attestedChat` surfaces `intelQuote` / `signingPublicKey` so a caller who needs that can do it out of band; see `veniceai/venice-cli` (`src/lib/tee.ts`) for a reference parser.
